 ...
 ******************************************************************************/
/******************************************************************************
 * SAM-Only Simplified Version
 *
 * This file contains a simplified version of the flash attention forward kernel
 * optimized specifically for SAM (Segment Anything Model) usage.
 *
 * REMOVED FEATURES (not used by SAM inference):
 * - Dropout (SAM inference never uses dropout)
 * - Causal masking (is_causal always false for SAM)
 * - Local window attention (not used in SAM)
 * - ALiBi positional bias (not used in vision models)
 * - Streaming attention (not used in SAM)
 * - Dense attention path (SAM only uses block-sparse)
 * - Return softmax values (training-only feature)
 * - RNG/Philox seed handling (dropout-related, not needed)
 *
 * KEPT FEATURES (essential for SAM):
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// Simplified softmax for SAM block-sparse attention
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// Main SAM block-sparse attention kernel
//
// Template parameters:
...
//
// Simplified from original by removing (inference-only):
//   - Is_dropout (no training)
//   - Is_causal (SAM is bidirectional)
//   - Is_local (SAM uses global block-sparse)
//   - Has_alibi (not used in vision)
//   - Is_streaming, Is_exact_streaming (SAM uses block-sparse)
//   - Return_softmax (training/debugging only)
////////////////////////////////////////////////////////////////////////////////////////////////////
...
inline __device__ void compute_block_attn_sam(
    ...
    // Initialize block-sparse mask iterator (always use fwdBlockmask for SAM)
    ...
    // SAM only uses non-causal, so n_masking_steps = 1
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (is_skip) {
            ...
            // Apply masking for OOB elements (SAM is non-causal, non-local)
            if (!Is_even_MN) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// Entry point kernel for SAM block-sparse attention (inference only)
////////////////////////////////////////////////////////////////////////////////////////////////////

template<typename Kernel_traits, bool Is_even_MN, bool Is_even_K, typename Params>
inline __device__ void compute_attn_sam(const Params &params) {
    const int m_block = blockIdx.x;
    const int bidb = blockIdx.y;
    const int bidh = blockIdx.z;

    // SAM only uses block-sparse attention (head_mask_type > 0)

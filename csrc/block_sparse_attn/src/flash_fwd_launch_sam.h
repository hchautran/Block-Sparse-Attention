 ...
 * Simplified SAM-only launcher - no dropout, no causal, inference only.
 ******************************************************************************/
...
#include "flash_sam.h"
#include "flash_fwd_kernel_sam.h"

namespace FLASH_NAMESPACE {

////////////////////////////////////////////////////////////////////////////////////////////////////
// Simplified kernel launcher for SAM
...
__global__ void flash_fwd_sam_kernel(Flash_fwd_sam_params params) {
    FLASH_NAMESPACE::compute_attn_sam<Kernel_traits, Is_even_MN, Is_even_K>(params);
}
...
void run_flash_fwd_sam(Flash_fwd_sam_params &params, cudaStream_t stream) {
    ...
    BOOL_SWITCH(is_even_MN, IsEvenMNConst, [&] {
        BOOL_SWITCH(is_even_K, IsEvenKConst, [&] {
            ...
            auto kernel = &flash_fwd_sam_kernel<
                Kernel_traits,
...
template<typename T>
void run_mha_fwd_sam_hdim32(Flash_fwd_sam_params &params, cudaStream_t stream) {
    constexpr static int Headdim = 32;
    // SAM doesn't use head_dim=32, but included for completeness
    run_flash_fwd_sam<Flash_fwd_kernel_traits<Headdim, 128, 128, 4, false, false, T>>(params, stream);
}

template<typename T>
void run_mha_fwd_sam_hdim64(Flash_fwd_sam_params &params, cudaStream_t stream) {
...
template<typename T>
void run_mha_fwd_sam_hdim128(Flash_fwd_sam_params &params, cudaStream_t stream) {
    ...
    if (is_sm8x) {
        ...
        run_flash_fwd_sam<Flash_fwd_kernel_traits<Headdim, 128, 32, 4, false, false, T>>(params, stream);
    } else {
        // For sm80 (A100), sm90 (H100): 128x64
        run_flash_fwd_sam<Flash_fwd_kernel_traits<Headdim, 128, 64, 4, false, false, T>>(params, stream);
    }
}

template<typename T>
void run_mha_fwd_sam_hdim256(Flash_fwd_sam_params &params, cudaStream_t stream) {
    constexpr static int Headdim = 256;
    // Larger head dimension: use smaller N blocks
    run_flash_fwd_sam<Flash_fwd_kernel_traits<Headdim, 64, 64, 4, false, false, T>>(params, stream);
}

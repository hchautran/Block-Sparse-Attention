...
of 128, and 16 attention heads. The softmax scale is set to 1.0. The computation
...
.. code-block:: bash
    ...
    ncu python examples/ampere/flash_attention_v2.py                                        \
        --dtype Float16 --head_dim 128 --m_block_size 128 --n_block_size 128                \
        --num_threads 128 --batch_size 1 --seqlen_q 1280 --seqlen_k 1536                    \
        --num_head 16 --softmax_scale 1.0 --skip_ref_check
...
class FlashAttentionForwardAmpere:
    ...
    def __call__(
        ...
        mBlockMask has dtype Int8 and layout:
        (batch_size, num_head, num_m_blocks, num_n_blocks) where
        num_m_blocks = ceil(seqlen_q / m_block_size), num_n_blocks = ceil(seqlen_k / n_block_size).
        A value of 1 means the block is computed; 0 means it is skipped.
        ...
        :type mO: cute.Tensor
        :param mBlockMask: block sparsity mask (Int8, 1=compute, 0=skip)
        :type mBlockMask: cute.Tensor
    ...
    def kernel(
        ...
        # ///////////////////////////////////////////////////////////////////////////////
        # Softmax intermediate result: row_max and row_sum — declared before the
        # early-exit guard so normalize_softmax in the epilogue always has access.
        # ///////////////////////////////////////////////////////////////////////////////
        ...
        # ///////////////////////////////////////////////////////////////////////////////
        # Early-exit: OR all mask entries for this m-block. When every n-block is
        # masked (row_mask_or == 0), skip the prologue and main loop entirely.
        # acc_O is already zero-filled; normalize_softmax handles zero row_sum by
        # writing zeros — giving the correct zero-attention output with no GEMM or
        # GMEM traffic. Especially effective for small seqlen with sparse patterns
        # (e.g. seqlen < 200 where n_block_max <= 2, so this check costs <= 2 loads).
        # ///////////////////////////////////////////////////////////////////////////////
        row_mask_or = mBlockMask[batch_size, num_head, m_block, 0]
        for n in range(1, n_block_max):
            row_mask_or = row_mask_or | mBlockMask[batch_size, num_head, m_block, n]
        ...
        if row_mask_or != 0:
            ...
            # ///////////////////////////////////////////////////////////////////////////
            # Prefetch Prologue: Q is always loaded. K is only prefetched when the
            # first n-block (n_block_max-1) is non-masked — masked blocks never read K,
            # so skipping avoids one full K tile GMEM load for that slot.
            # Both Q-only and Q+K loads are committed in a single cp_async group so
            # cp_async_wait_group(0) in compute_one_n_block always drains correctly.
            # ///////////////////////////////////////////////////////////////////////////
            ...
            if mBlockMask[batch_size, num_head, m_block, n_block] != 0:
            ...
            # First n-block needs padding-mask handling for seqlen_k residue.
    ...
    def compute_one_n_block(
        ...
        # ///////////////////////////////////////////////////////////////////////////////
        # Block sparse: check mask before any heavy computation.
        # mBlockMask is (batch, head, m_block, n_block); 1 = compute, 0 = skip.
        # The condition is CTA-uniform, so all threads take the same branch.
        # ///////////////////////////////////////////////////////////////////////////////
        is_masked = (
            mBlockMask[
                basic_params.batch_size,
                basic_params.num_head,
                basic_params.m_block,
                basic_params.n_block,
            ]
            == 0
        )

        # Drain any K prefetch committed in the previous iteration. Unconditional:
        # cp_async_wait_group(0) is a no-op when no group is pending (which happens
        # when the previous block skipped its K prefetch because this block is masked).
        cute.arch.cp_async_wait_group(0)
        self.cta_sync_barrier.arrive_and_wait()

        if not is_masked:
            # ///////////////////////////////////////////////////////////////////////////
            # Load V and run S = Q * K^T only for non-masked blocks.
            # Skipping these saves V GMEM bandwidth and tensor-core cycles.
            # ///////////////////////////////////////////////////////////////////////////
            ...
            # wait for smem tile V for O (only needed when V was actually loaded)
            cute.arch.cp_async_wait_group(0)
            self.cta_sync_barrier.arrive_and_wait()

        # Prefetch K for the next n-block only when that block is non-masked.
        # Masked blocks never read K, so skipping saves K GMEM bandwidth proportional
        # to sparsity. cp_async_wait_group(0) at the start of the next call handles
        # both cases: drains the group if committed, is a no-op if not.
        ...
        if not is_masked:
            # ///////////////////////////////////////////////////////////////////////////
            # online softmax and O = P * V only for non-masked blocks.
            # ///////////////////////////////////////////////////////////////////////////
    ...
    def softmax_rescale_O(
        ...
        for r in cutlass.range_constexpr(cute.size(softmax_params.row_max)):
            ...
            if cutlass.const_expr(in_mask_steps):
                # mask padding columns of the last K tile with -inf
            ...
            if cutlass.const_expr(not is_first_n_block):
                ...
                # Guard: if this is the first block and all entries are -inf (fully
                # masked block), clamp row_max to 0 to avoid exp2(-inf - (-inf)) = NaN.
                row_max_cur_row = (
...
def run(
    ...
    **kwargs,
    ...
    if not FlashAttentionForwardAmpere.can_implement(
        ...
        raise TypeError(
            f"Unsupported testcase {dtype}, {head_dim}, {m_block_size}, {n_block_size}, {num_threads}"
        )
    ...
    # Block sparse mask: (batch, head, num_m_blocks, num_n_blocks), Int8.
    # 1 = compute block, 0 = skip block.  sparsity=0.0 → dense (all ones).
    num_m_blocks = (seqlen_q + m_block_size - 1) // m_block_size
    num_n_blocks = (seqlen_k + n_block_size - 1) // n_block_size
    if sparsity > 0.0:
        block_mask_torch = (
            torch.rand(batch_size, num_head, num_m_blocks, num_n_blocks) >= sparsity
        ).to(dtype=torch.int8).cuda()
    else:
        block_mask_torch = torch.ones(
            batch_size, num_head, num_m_blocks, num_n_blocks, dtype=torch.int8
        ).cuda()
    block_mask = from_dlpack(block_mask_torch, assumed_align=1)
    ...
    compiled_fa2_fwd = cute.compile(
        fa2_fwd, q, k, v, o, block_mask, softmax_scale, current_stream,
        options=compile_options,
    )

    if not skip_ref_check:
        compiled_fa2_fwd(q, k, v, o, block_mask, softmax_scale, current_stream)
        torch.cuda.synchronize()
        import torch
        q_ref = q_torch.permute(0, 2, 1, 3).float()  # (B, H, Sq, D)
        k_ref = k_torch.permute(0, 2, 1, 3).float()  # (B, H, Sk, D)
        v_ref = v_torch.permute(0, 2, 1, 3).float()  # (B, H, Sk, D)
        # Build a block-level attention bias from the sparse mask.
        # attn_bias[b, h, q_i, k_j] = 0 if the block is active, -inf if masked.
        attn_bias = torch.zeros(
            batch_size, num_head, seqlen_q, seqlen_k, dtype=torch.float32
        )
        for mb in range(num_m_blocks):
            for nb in range(num_n_blocks):
                q_start = mb * m_block_size
                q_end = min(q_start + m_block_size, seqlen_q)
                k_start = nb * n_block_size
                k_end = min(k_start + n_block_size, seqlen_k)
                # block_mask_torch: (B, H, num_m_blocks, num_n_blocks)
                mask_val = block_mask_torch[:, :, mb, nb]  # (B, H)
                # Expand to tile and apply
                attn_bias[:, :, q_start:q_end, k_start:k_end] = torch.where(
                    mask_val[:, :, None, None] == 0,
                    torch.full((), float("-inf")),
                    torch.zeros(()),
                )
        attn_bias = attn_bias.cuda()
        # Reference: scaled dot-product attention with block-sparse bias
        scores = torch.matmul(q_ref, k_ref.transpose(-2, -1)) * softmax_scale
        scores = scores + attn_bias
        ref_o = torch.nn.functional.softmax(scores, dim=-1)
        # Handle rows where all keys are masked (softmax of all-inf → nan → 0)
        ref_o = torch.nan_to_num(ref_o, nan=0.0)
        ref_o = torch.matmul(ref_o, v_ref)
        ref_o = ref_o.permute(0, 2, 1, 3).to(dtype=cutlass_torch.dtype(dtype))
    ...
    def generate_tensors():
        ...
        mask_workspace = from_dlpack(
            torch.ones(batch_size, num_head, num_m_blocks, num_n_blocks,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seqlen_q", type=int, default=200)
    parser.add_argument("--seqlen_k", type=int, default=200)
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Fraction of (m_block, n_block) pairs to skip (0.0 = dense, 0.5 = 50%% sparse)",
    )

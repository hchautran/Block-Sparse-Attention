"""
SAM Block Sparse Attention - Simplified Python Interface

This module provides a simplified, SAM-specific interface for block sparse attention.
Optimized for inference only with the following assumptions:
- No dropout (always p_dropout=0)
- No causal masking (SAM is bidirectional)
...
import torch
import block_sparse_attn_sam_cuda  # The compiled C++ extension
...
def sam_block_sparse_attn(
    ...
    """
    SAM-specific block sparse attention (inference only).
...
def sam_block_sparse_attn_simple(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    base_blockmask: Optional[torch.Tensor],
...
def block_sparse_attn_sam_func(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    cu_seqlens_q: torch.Tensor,
    ...
    """
    Alias for sam_block_sparse_attn for backward compatibility.
    """
    return sam_block_sparse_attn(
        q, k, v,
        cu_seqlens_q, cu_seqlens_k,
